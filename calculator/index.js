...
options.addEventListener('change', (e) => {
        ...
        if ( options.value == 'Table' || options.value == 'Factors') {
            ...
            changeInput( 4, 
                ['Principle', 'Rate', 'Time (no. of years)', 'No. of times/year'], inputArr)
        } 
...
for (const btn of arithmetic_btn) {
    ...
    btn.addEventListener('click',()=>{
        ...
        let val = calculator(window[btn.value], num1Value, num2Value, num3Value, num4Value);
        loadEle(val);
    })
}
...
calculate_btn.addEventListener('click', (e) => {
    ...
    if(options.value == 'HCF' || options.value == 'LCM'){
        ...
        let num3Value = Number(numberInputs[2].value)
        let num4Value = Number(numberInputs[3].value);
        val = calculator(window[options.value], num1Value, num2Value, num3Value, num4Value);
    }
    loadEle(val);  
})
...
function Divide(a, b) {
    let val = a / b
    num1 = val
    return `${val} <p> Quotient : ${Math.floor(val)} </p><p>Remainder : ${a % b} </p>`
}
function Power(a, b) {
    let val = a ** b
    num1 = val
...
function HCF(digits) {
    ...
    commonFactors = factors[0].filter(elem => factors.every(array => array.includes(elem)));
...
function LCM(digits) {
    let lcm = 1, res = 1;
    let product = 1;let hcf_product = 1;
