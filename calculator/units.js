  ...
  let measureType = measurement_Type.value;
      ...
      if ( units.mode_am_pm != undefined ){
            ...
            for (let i=0; i < measure_1.length; i++) {
                 ...
                 measure_1[i].setAttribute('title',`Time must be between ${units.min[i]} & ${units.max[i]}`)
            }
      }
      else{ 
...
for (const btn of measureCalc_btns) {
      ...
      for (const ele of numInputs) {
            if ( ! ele.checkValidity()){
                 ...
                 error.innerHTML = 
                 `<b>'${ele.placeholder}'</b>. &nbsp; ` + ele.validationMessage + 
                 `<br><img src=".assets/images/brain_confused.jpg" width="120px"> `;
...
console.log( toNormalUnits(37000, {hour:0,minute:10,second:40}, TimeSys, 'second') );

function toNormalUnits( val, modelArr, sys = metricSys, base = 'unit' ){
      let res = 0, resArr = {}, keys = Object.keys(modelArr)
...
// all 4 operations
        let res = 0;
        let val1 = valOf( inp1, metricSys, 'unit'); let val2 = 0
...
function toTimeUnits( val , mode = 'none'){
        ...
        let duration = toNormalUnits(val, {hour:0,minute:0,second:0}, TimeSys, 'second');
